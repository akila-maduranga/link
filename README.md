...
| **Platform landing pages** | `/explore/[platform]` | Server-rendered listings (no client fetch waterfall) with unique per-platform titles ("Telegram Channels & Groups Directory – Find Best Telegram Communities \| FindLink" format), meta descriptions covering platform + filters + shortener benefit, and unique 100–150-word intro copy — the pages that target "telegram channels", "whatsapp group links", "discord servers" style searches |
| **Listing pages** | `/link/[slug]` | Every public listing is SEO-ready from day one: unique title ("[Community Name] – [Platform] [Channel/Group] \| FindLink"), meta description built from the submitted description (word-safe ~155-char truncation), H1 = community name, 5-crumb breadcrumb (Home > Explore > Platform > Category > Listing) and a category-aware "Related communities" section |
| **Structured data (JSON-LD)** | every public page | `WebSite` + `SearchAction` (sitelinks search box), `Organization`, `BreadcrumbList`, `CollectionPage`/`ItemList` on directory pages, `WebPage` + `CreativeWork` on listing pages, `Product`+`Offer` on `/premium`. All server-rendered with absolute URLs |
...
| An update doesn't fully take effect (e.g. `/robots.txt` still shows the old static content) | Unzipping a new zip **over** an existing clone never *deletes* files the new zip no longer contains — stale files survive and get pushed. Known instance: `public/robots.txt` (removed in the SEO update — Next.js serves a `public/` file in preference to the `app/robots.ts` route). Fix in your repo folder: `rm -f public/robots.txt && git add -A && git commit -m "Remove stale robots.txt" && git push`, wait for the **Actions** build, then `./install.sh --update`. Future-proof: after any zip overwrite, `git status` should be empty — if it shows deleted files still tracked, remove them the same way |

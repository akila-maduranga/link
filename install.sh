...
#      ./install.sh [--domain findlink.site] [--resend-key re_xxx] [--port 3000]
#
...
#      --domain   FQDN  Your domain, e.g. findlink.site (sets APP_URL)
...
REPO_URL=""; DOMAIN=""; RESEND_KEY=""; PORT="3000"; ASSUME_YES="false"; UPDATE="false"
while [[ $# -gt 0 ]]; do
  case "$1" in
...
if [[ -f "$ENV_FILE" ]]; then
  ...
  if [[ -n "$DOMAIN" ]]; then
    ...
    APP_URL="http://$(hostname -I 2>/dev/null | awk '{print $1}' || echo localhost):${PORT}"
  fi
...
echo -e "   ${CYAN}docker logs -f findlink${RESET}            # follow logs"
...
echo -e " ${YELLOW}Domain:${RESET} point an A record (e.g. findlink.site) at this VPS, set"
echo -e " APP_URL in .env, then put a reverse proxy (Caddy/Nginx/Cloudflare) in"
echo -e " front of port ${PORT}."
echo -e "${BOLD}──────────────────────────────────────────────────────────${RESET}"
echo ""
